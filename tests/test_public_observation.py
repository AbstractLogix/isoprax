...
def snapshot(**changes):
    return replace(PublicObservationSnapshot("system-a", "https://example.test/observation", "threshold-v1", "2026-09-06T00:00:00Z", "2026-09-06T00:10:00Z", ("result.json",), True, True), **changes)


def test_normalizes_deterministically():
    second = snapshot(system_id="system-b", score_time="2026-09-07T00:00:00Z", window_end="2026-09-07T00:10:00Z")
    assert normalize_public_observations([second, snapshot()]) == normalize_public_observations([snapshot(), second])
    assert normalize_public_observations([snapshot()])[0].outcome_class == "observed"


@pytest.mark.parametrize("changes,reason", [({"artifacts_complete": False}, "incomplete"), ({"valid": False}, "invalid")])
