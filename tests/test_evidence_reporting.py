...
def splits():
    return (
        ...
        SplitDefinition(
            "calibration_fit", "2026-09-10T00:00:00+00:00", "2026-09-15T00:00:00+00:00"
        ),
        SplitDefinition(
            "calibration_gate", "2026-09-15T00:00:00+00:00", "2026-09-20T00:00:00+00:00"
        ),
        SplitDefinition(
...
def report_profile(assembly):
    return EvidenceReportProfile(
        assembly.profile_identity,
        "public synthetic replay evidence",
        "predeclaration-hash",
        "anchor://predeclaration",
        ("manifest.json",),
        ("build_qualification", "runner_execution", "capture_artifacts"),
    )
...
def admission_profile():
    return AdmissionProfile(
        ...
        corpus_provenance=CorpusProvenance("system-a", False, False),
...
def test_builds_deterministic_safe_report():
    ...
    first = build_evidence_report(
        report_profile(assembled), assembled, [capture()], profile, admission
    )
...
def test_reports_unavailable_and_censoring_without_upgrading_claims():
    ...
    result = build_evidence_report(
        ...
        [censored],
        profile,
        evaluate_admission(list(assembled.rows), profile),
    )

    assert result.status == "blocked"
    assert result.counts["censored"] == 1
    assert any(
        item.category == "capture_artifacts" for item in result.unavailable_evidence
    )
...
def test_distinguishes_complete_admission_evidence_from_inconclusive():
    ...
    assert (
        build_evidence_report(
            complete_profile, assembled, [capture()], profile, passed
        ).status
        == "admission_evidence"
    )
    assert (
        build_evidence_report(
            report_profile(assembled), assembled, [capture()], profile, passed
        ).status
        == "inconclusive"
    )
...
def test_rejects_unsafe_reporting_inputs(
    profile_change, assembly_change, capture_change, admission_profile_change
):
    assembled, profile, admission = evidence()
    with pytest.raises(ValueError):
        build_evidence_report(
            replace(report_profile(assembled), **profile_change),
            replace(assembled, **assembly_change),
            [replace(capture(), **capture_change)],
            replace(profile, **admission_profile_change),
            admission,
        )
...
def test_rejects_remaining_profile_and_capture_contract_failures():
    ...
    with pytest.raises(ValueError):
        EvidenceReportProfile(
            "id", "scope", "hash", "anchor", ("artifact", "artifact"), ("runner",)
        )
    cases = (
        (
            replace(
                assembled,
                manifest_input={**assembled.manifest_input, "published_artifacts": ()},
            ),
            [capture()],
            profile,
        ),
        ...
        (
            ...
            [
                replace(
                    capture(),
                    observation=replace(
                        capture().observation, uses_private_production_data=True
                    ),
                )
            ],
            profile,
...
def test_collected_artifacts_complete_the_required_evidence_category():
    ...
    complete_capture = replace(
        capture(),
        artifacts=(
            ObservationArtifactEvidence("result.json", "collected", "a" * 64, 1),
        ),
    )
    passed = AdmissionReport(True, (), admission.counts_by_split, admission.manifest)
    result = build_evidence_report(
        report_profile(assembled), assembled, [complete_capture], profile, passed
    )
