...
- **FR-005**: System MUST disqualify a candidate at Screen 3 if a sampled build-attempt success rate falls below the candidate's predeclared build floor, where the build floor MUST be frozen before screening and set from a fixed reference sample of the most recent 200 known-good commits from an unrelated stable project. The floor MUST be computed as the 5th-percentile observed build-success rate across that sample, rounded down to two decimal places, and MUST NOT be set above the lower of that reference-rate floor and 0.90. If build failures cluster in time such that dropping them would bias the corpus, the candidate is also disqualified.
